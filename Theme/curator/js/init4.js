const version_of_curator_init="15"
const site = "http://userspace.org";       

console.log("VERISON:"+version_of_curator_init );

function showLoadingCollection() {

    const spcer = "&nbsp;</BR>".repeat(20);
    document.body.querySelector("#collections").innerHTML = spcer;
    document.body.querySelector("#collections").sparkle({
        position: "afterbegin",
        count: 30, 
        minAge: 500,
        maxAge: 2000,
...
function hideLoadingCollection() {
...
let timeout_this = new Promise((resolve, reject) => {
  let wait = setTimeout(() => {
    //clearTimout(wait);
    errorLoadingCollection();
    console.log("TIMEOUT");
    resolve('TIMEOUT');
  }, 60000)
})

function embedcollection( site_url ) {
showLoadingCollection(); 
...
fetch( site+site_url )
       ...
       .catch( hideLoadingCollection  )
       .finally( hideLoadingCollection ),
timeout_this		
])
...
switch(e.target.id) {
  ...
  case "NEWS":
    ...
    embedcollection( '/newsfeeds' );
    return false;
    break;
  case "VIDEO":
    //console.log("SELECT VIDEO");
    embedcollection( '/videocast' );
    return false;
    break;
  case "AUDIO":
    //console.log("SELECT AUDIO");
    embedcollection( '/podcast' );
    return false;
    break;
  case "INFOSEC":
    //console.log("SELECT INFOSEC");
    embedcollection( '/infosec' );
    return false;
    break;
  default:
    return false;
